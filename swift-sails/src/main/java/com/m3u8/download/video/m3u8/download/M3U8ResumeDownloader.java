...
public class M3U8ResumeDownloader {
    ...
    //所有ts片段下载链接
    private Set<DownloadSegment> tsSet = new LinkedHashSet<>();
    ...
    // 已下载url
    private Set<String> downloadedUrl;
    // 是否监听
    private volatile boolean isListening = true;

    private DownloadTask downloadTask;
    private boolean pause;
    private DownloadSegmentService downloadSegmentService = SpringUtil.getBean(DownloadSegmentService.class);;

    public M3U8ResumeDownloader(int threadCount, String dir, String tempPath, String fileName, int finishedCount,
                                Set<File> finishedFiles, String key, byte[] keyBytes, boolean isByte,
                                BigDecimal downloadBytes, BigDecimal totalFileSize,
                                Set<DownloadListener> listenerSet, String suffix,
                                boolean isListening, boolean pause, Set<DownloadSegment> tsSet) {
...
//            getM3u8FileSize();
        //执行多线程下载
        for (DownloadSegment downloadSegment : tsSet) {
            if (downloadedUrl.contains(downloadSegment.getSegmentPath())) {
                continue;
            }
            fixedThreadPool.execute(getThread(downloadSegment));
        }
...
//                         打开视频文件
    ...
    private void startListener(ExecutorService fixedThreadPool) {
        new Thread(() -> {
            for (DownloadListener downloadListener : listenerSet)
                downloadListener.start(downloadTask);
            //轮询是否下载成功
            while (!fixedThreadPool.isTerminated() && isListening) {
                try {
     ...
     * @return 线程
     */
    private Thread getThread(DownloadSegment downloadSegment) {
        return new Thread(() -> {
            int count = 1;
            HttpURLConnection httpURLConnection = null;
            //xy为未解密的ts片段，如果存在，则删除
            File file2 = new File(TEMP_PATH + FILESEPARATOR + downloadSegment.getSegmentNumber() + ".xy");
            if (file2.exists()) {
                file2.delete();
            }
            ...
            while (count <= retryCount) {
                ...
                try {
                    //模拟http请求获取ts片段文件
                    URL url = new URL(downloadSegment.getSegmentPath());
                    if (proxy == null) {
                    ...
                    File file = new File(TEMP_PATH + FILESEPARATOR + downloadSegment.getSegmentNumber()
                            + ".xyz");
                    ...
                    synchronized (this) {
                        downloadBytes = downloadBytes.add(new BigDecimal(bytes.length));
                    }
                    finishedFiles.add(file);
                    finishedCount++;
                    synchronized (downloadSegment){
                        downloadSegment.setStatus(DownloadStatusEnum.COMPLETED.get());
                        downloadSegment.setSaveDirectory(file.getParent());
                        downloadSegmentService.updateDownloadSegment(downloadSegment);
                        System.out.println("已更新数据库：" + downloadSegment);
                    }
                    break;
                } catch (Exception e) {
                    ...
                    Log.d("第" + count + "获取链接重试！\t" + downloadSegment.getSegmentPath());
                    count++;
//                        e.printStackTrace();
                } finally {
                    try {
            ...
            if (count > retryCount) {
                //自定义异常
                connectFailUrl.put(downloadSegment.getSegmentNumber(), downloadSegment.getSegmentPath());
    ...
    public Set<String> getDownloadedUrl() {
        return downloadedUrl;
    }

    public void setDownloadedUrl(Set<String> downloadedUrl) {
        this.downloadedUrl = downloadedUrl;
    }

    private void mergeAndDeleteListenerEvent() {
        for (DownloadListener downloadListener : listenerSet) {
            downloadListener.mergeAndDelete();
        }
    }
    ...
    public Set<DownloadSegment> getTsSet() {
        return tsSet;
    }

    public void setTsSet(Set<DownloadSegment> tsSet) {
        this.tsSet = tsSet;
    }
